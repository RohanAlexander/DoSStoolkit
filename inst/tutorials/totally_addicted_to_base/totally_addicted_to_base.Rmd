...
GitHub. The Broadway Grosses data set comprises data on revenue and attendance figures 
for theaters that are part of The Broadway League.
...
The function consist of four parts: 
...
### While Loop
There is another way to create loops, using `while` statement. `while` loops are used to loop until a specific condition is met/satisfied. In for loops, we already know how many times the loop will run. However, `while` can be used when the exact number of iterations is not known.  
...
The if statement is a tool for returning an output based on a logical condition. The syntax of the if statement is:
...
if_else statement has two parts. The first part is exactly the same as the if statement, and the second part is the else statement. The else statement will be evaluated if the condition is FALSE. 
...
## hist() and other base graphing options

Written by Yena Joo.

### Introduction

R has various functions to produce a wide range of graphics to visualize data. With the basic functions that R provides, we can easily and quickly make different types of graphs within seconds.  
...
Changing the colour, labeling a title and axes is easy, the same as how we did for histograms and scatterplots. 
use `horizontal=TRUE` to reverse the axis orientation.  

Detailed descriptions of the arguments could be found in `help("boxplot")` or at:   
