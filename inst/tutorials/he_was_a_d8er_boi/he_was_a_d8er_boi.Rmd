...
Here I have set the n equal to 3, so we are looking at the first three row of the mtcars dataset. 
...
Here I have set the n equal to 3, so we are looking at the last three row of the mtcars dataset. 
...
### Next Steps
Once you have fully understood the dataset you are working with. You may start using plots to get a graphical representation of your dataset. 
You may read this chapter for more information! https://r4ds.had.co.nz/data-visualisation.html 
...
**1. Using the `presidential` data frame, save the column names, and then change the name of the second column to `inauguration-date**
...
**4. Bind the `presidential` data set to itself using `rbind()`.** 
...
**6. Bind the `presidential` data set to itself using `cbind()`.** 
...
- Join two tables by using left_join(), right_join(), full_join() 
...
Sometimes you may want to combine two data frames into a single table.
Here we have one table which contains data such as the student id and their grade. And we have another table that includes demographic information about the student.
...
```{r leftjoin}
leftJoinDf <- left_join(test_score,student_info,by='student_id')
leftJoinDf
```
...
- Make sure there is at least one common variable in the tables you joining
...
```{r}
set.seed(2)
sample(x, 6, replace = TRUE, prob = c(0.12, 0.12, 0.12, 0.12, 0.12, 0.4))
```
...
- Familarity with conditional statements (_coming soon!_)
...
```{r factor-levels}
factor_vector <- c(1,2,3)
factor(factor_vector, levels = c(2,1,3)) ## set levels to 2,1,3
```
...
```{r factor-labels}
factor_vector <- c(1,2,3)
factor(factor_vector, labels = c('one', 'two', 'three')) ## set labels to 'one', 'two', 'three'
```

Using `exclude` we can exclude the number two from our factor.
```{r factor-exclude}
factor_vector <- c(1,2,3)
factor(factor_vector, exclude = 2)
```
...
```{r is-factor}
factor_vector <- c(1,2,3)
new_factor <- factor(factor_vector, exclude = 2)

is.factor(new_factor)
```
...
factor_vector <- c(1,2,3)
new_factor <- factor(factor_vector)
...
```{r fct-count}
random_factor <- factor(c(1,1,1,2,3,4,2,3,2,1,4,3,3,3,2,1))
...
```{r fct-lump}
random_factor <- factor(c(1,1,1,2,3,4,2,3,2,1,4,3,3,3,2,1,1))

fct_lump_n(random_factor, 2) ## keep the 2 most frequent levels
fct_lump_prop(random_factor, .25) ## lump levels which appear less than 25 % of time
fct_lump_min(random_factor, 3) ## lump levels which appear less than 3 times
```
...
```{r fct-relevel}
temperatures <- factor(c("hot", "cold", "medium"))
levels(temperatures)
...
| -------- | --------- | ----------------------------------------- |
| data     | data frame | A data frame |
| ... | string | The name(s) of the column(s) containing the data to be separated. Multiple columns can be inputted if their lengths are compatible (ie. the same number of new rows will be created once they are separated). Otherwise, an error will be raised. |
...
If yo would like to learn more about handling dates and times in R, as well as the
