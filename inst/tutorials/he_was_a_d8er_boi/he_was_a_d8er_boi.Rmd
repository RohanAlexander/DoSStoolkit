...
- Making use of lubridates today() and now() functions
...
dont always get to choose our column names (because most times, we are not creating our
own data, but were getting data from another source), so sometimes we might end up
with column names that aren not easy to work with.
...
Wickhams Tidy Data paper](https://vita.had.co.nz/papers/tidy-data.html) from 2014, Hadley
introduces 3 rules every dataset must follow in order to be considered tidy.

This is a good paper to read, but if you dont have time, here is an illustration that
...
its natural and theres a lot to digest, especially because datasets can be so uniquely
messy (as Hadley Wickhams quote says in the picture above)
...
Suppose youre on your first data internship, and your manager gives you your first
...
The first thing you do, of course, is load and view the file in R. As you do this, youre
immediately confused by the structure. This isnt what you've seen in your data courses!
The variables are rows not columns, and the observations are columns not rows! You take a
deeper look and you see that some variables dont  show the data. It looks like
theyre hidden behind some text. Lets take a look at an example of this data.

Suppose you're given this data that you know is not in tidy format. 
...
This data is not in its tidy format. First, the columns and rows are switched, and
second, the cells are all hidden.

Heres the entire piece of code we need to tidy it:
...
But lets go through this step by step and check the output each time.
